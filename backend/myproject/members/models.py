...
class FileModel(models.Model):
    ...
    related_project = models.ForeignKey(
        "projects.ProjectModel", on_delete=models.CASCADE, null=True, blank=True
    )
    related_submission = models.ForeignKey(
        "projects.SubmissionModel", on_delete=models.CASCADE, null=True, blank=True
    )
    ...
    def clean(self):
        ...
        if self.type == self.PDF and not (
            self.file and self.file.name.endswith(".pdf")
        ):
            raise ValidationError("File must be a PDF.")
        elif self.type == self.IMAGE and not (
            self.image and self.image.name.endswith((".jpg", ".jpeg", ".png", ".gif"))
        ):
